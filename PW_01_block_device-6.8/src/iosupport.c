...
static void PwbdpDevOpsSubmitBio(struct bio *Bio)
{
    ...
    pr_info_tl(PWBD_TL_3, "completing with %u (%s) device 0x%px (%u)", Bio->bi_status,
               blk_status_to_str(Bio->bi_status), device, device->DeviceNumber);
...
static int PwbdpDevOpsOpen(struct gendisk *Disk, blk_mode_t Mode)
{
    ...
    pr_info_tl(PWBD_TL_3, "Disk 0x%px Mode 0x%08X device 0x%px (%u)", Disk, Mode, device,
               device->DeviceNumber);
...
static void PwbdpDevOpsRelease(struct gendisk *Disk)
{
    ...
    pr_info_tl(PWBD_TL_3, "Disk 0x%px openers %u device 0x%px (%u)", Disk, disk_openers(Disk),
